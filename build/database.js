...
function createUser(id, email, password) {
    const userVerification = users.find((user) => user.email === email || user.id === id);
    if (userVerification) {
...
function queryProductsByName(q) {
    const queryProducts = products.filter((product) => product.name.toLowerCase() === q.toLowerCase());
...
getAllUsers();
console.log('\n---Create Product--- \n');
createProduct('1', 't-shirt', 40, types_1.CATEGORY.CLOTHES_AND_SHOES);
createProduct('2', 'necklace', 80, types_1.CATEGORY.ACCESSORIES);
console.log('\n---Get All Products---\n');
getAllProducts();
console.log('\n---Get Product By Id--- \n');
getProductById('1');
